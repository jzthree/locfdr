...
\item 3 gives
          ...
          each bin) with respect to the bin counts.
...
pds. is the derivative of the estimates of p0 (when
            \texttt{nulltype==1}) or \texttt{log(p0)} (when
            \texttt{nulltype==0} or \texttt{2}), delta, and sigma with
...
\texttt{http://www-stat.stanford.edu/}$\tilde{\mbox{\,\,\,\,\,}}$\texttt{brad/papers/Correlation-2006.pdf}
\end{thebibliography}
