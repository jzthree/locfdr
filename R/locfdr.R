    ...
    type = 0, plot = 1, mult, mlests, main = " ", sw = 0, LRfdr=False) 
{
    ...
    if (LRfdr){
      fFdrl <- approx(x, Fdrl, zz, rule = 2, ties = "ordered")$y
      fFdrr <- approx(x, Fdrr, zz, rule = 2, ties = "ordered")$y
      vl = list(fdr = ffdr, fFdrl=fFdrl, fFdrr=fFdrr, fp0 = fp0, Efdr = Efdr, cdf1 = cdf1, 
          mat = mat, z.2 = z.2)
    }
    else{
